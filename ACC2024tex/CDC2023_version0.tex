...
\end{definition}
% \begin{lemma}
%     For any positive definite matrices $T,S,V_{1},V_{2}$, suppose $\lambda_{min}(S+V_{2}-T-V_{1}) > 0$. There exists a positive definite $\bar{V}$ that 
%     \begin{equation*}
%         \delta_{\infty}(T+V_{1}, S+V_{2}) \leq \delta_{\infty}(T+\bar{V},S+\bar{V}).
%     \end{equation*}
% \end{lemma}
% \begin{proof}
%     Construct $\bar{V}$ such that
%     \begin{equation}\label{eq:barV}
%         \lambda(\bar{V}) \geq \frac{\lambda_{max}(V_{1})\lambda_{max}(S)-\lambda_{min}(V_{2})\lambda_{min}(T)}{\lambda_{min}(S+V_{2}-T-V_{1})},
%     \end{equation}
%     and
%     \begin{equation}
%         \lambda(\bar{V}) > 0.
...
%     Then we have
%     \begin{align*}
%         (x^{\transpose}\bar{V}x)(x^{\transpose}[S+V_{2}-T-V_{1}]x) &\geq \lambda(\bar{V})\lambda_{min}(S+V_{2}-T-V_{1})\\
%         &\geq \lambda_{max}(V_{1})\lambda_{max}(S)-\lambda_{min}(V_{2})\lambda_{min}(T) \\
%         &\geq (x^{\transpose}V_{1}x)(x^{\transpose}Sx)-(x^{\transpose}Tx)(x^{\transpose}V_{2}x),
%     \end{align*}
%     for all $\|x\| = 1$. Rearrange the equation above, we have
%     \begin{align*}
%         &(x^{\transpose}\bar{V}x)(x^{\transpose}[S+V_{2}]x) + (x^{\transpose}Tx)(x^{\transpose}V_{2}x) + (x^{\transpose}Tx)(x^{\transpose}Sx) \geq \\
%         &(x^{\transpose}V_{1}x)(x^{\transpose}Sx) + (x^{\transpose}\bar{V}x) + (x^{\transpose}(T+V_{1}))x) + (x^{\transpose}Tx)(x^{\transpose}Sx),
%     \end{align*}
%     for any $\|x\| = 1$. The above is achieved by adding term $(x^{\transpose}Tx)(x^{\transpose}Sx)$ to both sides.
%     We further simplify the above inequality, we have that
%     \begin{align*}
%         (x^{\transpose}(T+\bar{V})x)((x^{\transpose}(S+V_{2})x)) \geq (x^{\transpose}(S+\bar{V})x)((x^{\transpose}(T+V_{1})x)).
%     \end{align*}
%     Thus,
%     \begin{equation}
%         \frac{x^{\transpose}(T+\bar{V})x}{x^{\transpose}(S+\bar{V})x} \geq \frac{x^{\transpose}(T+V_{1})x}{x^{\transpose}(S+V_{2})x}.
%     \end{equation}
%     By applying the maximum operator $\max_{\|x\|=1}$ to both sides of the inequality above, we can thus conclude that
%     \begin{align*}
%         \delta_{\infty}(T+\bar{V},S+\bar{V}) \geq \delta_{\infty}(T+V_{1},S+V_{2}).
%     \end{align*}
% \end{proof}
